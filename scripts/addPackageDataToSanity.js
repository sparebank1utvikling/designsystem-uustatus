...
// Hard coded for now, but should be fetches from a function or from the ffe-versions repo
const PackageData = [
    {
...
function addPackageData(packageName, packageVersions){
    const packageId = packageName.substring(5);
    client
    .transaction()
    .createIfNotExists({
        _id: packageId,        
    ...
    .patch(packageId, p => p.setIfMissing({ npm_package_versions: packageVersions }))
    .commit()
    .then((res) => {
        console.log("Everything updated", res);
    })
    .catch((err) => {
...
PackageData.map(package => { 
    addPackageData(package.name, package.versions)
})
